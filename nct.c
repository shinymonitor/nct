...
int main(int argc, char** argv){
    ...
    else if(strcmp(argv[1], "help")==0) {printf("Usage: nct [command] [options]\n\n"
        "Commands:\n"
        "\tinit  init [project] initializes a project with a given name and make .nct/.nct\n"
        "\tbuild Build project using build.c\n"
        "\trun   Run the built executable\n"
        "\ttest  Build and run test.c\n"
    ...
    if(strcmp(argv[1], "run")==0) {
        ...
        if(strcmp(last_modified_test, get_last_modified_test)!=0) if(system(compile_test_command)) {
            printf("Couldn't run compile test command properly.\n"); fputs(last_modified_test, file); fputs(last_modified_build, file);
        ...
        if(!file) {printf("Couldn't open .nct/.nct. Make sure that a valid .nct/.nct exist or create one using \'nct init\'\n"); return 1;}
        fprintf(file, "%s%s%s%s%s", compile_test_command, run_test_command, compile_build_command, run_build_command, run_bin_command);
        if(strcmp(last_modified_build, get_last_modified_build)!=0) if(system(compile_build_command)) {
